...
最终路由必须由插件 `scripts/workflow-runtime.mjs route --host codex` 根据结构化 `route_facts` 计算。AI 负责从需求与代码库提取事实，不得自行覆盖脚本结果；同一 facts、profile 状态和工作流版本必须得到相同 `route_fingerprint`。
...
- `.codex/ai-coding-workflow/project-profile.yaml` 的 `profile_status` 为 `ready`
