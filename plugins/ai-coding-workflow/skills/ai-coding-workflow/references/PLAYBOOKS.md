...
- 默认 profile 只安装 `SessionStart + Stop`：SessionStart 只注入活动 change、current node、phase 和状态路径；Stop 在仍有可执行 REQUIRED 节点时阻止停止，人工 Gate、真实 blocked 或 stop_hook_active 时允许返回。
- strict profile 额外安装 `PreToolUse(Edit|Write)`：planning/human-gate 阶段只允许修改当前 OpenSpec change；Fast planning 阶段不允许写文件。
- 不安装 PreCompact：runtime 已在节点转换、Review、重路由和归档时原子持久化，额外压缩 Hook 不增加可靠性。
- `install-hooks` 会精确对账本工作流管理的 Hook；切换 profile 或升级时不删除项目自有 Hook。

Hook 只执行快速确定性检查，不自动格式化、不运行全项目测试、不强制 tmux；默认 profile 不拦截文件写入。
