 ...
 * 状态迁移、评审范围校验，以及 Claude Code/Codex 生命周期 Hook。
 */
...
function normalizeHost(value) {
  const host = String(value ?? (process.env.PLUGIN_ROOT ? "codex" : "claude")).toLowerCase();
  if (!["claude", "codex"].includes(host)) throw new Error("--host 必须为 claude 或 codex");
  return host;
}

function workflowSupportDirectory(root, host) {
  return host === "codex"
    ? join(root, ".codex", "ai-coding-workflow")
    : join(root, ".claude");
}
...
function installHooks(root, options) {
  ...
  if (!["default", "strict"].includes(profileName)) throw new Error("--profile 必须为 default 或 strict");

  const fragment = readJson(fragmentPath);
  const settings = existsSync(settingsPath) ? readJson(settingsPath) : {};
  settings.hooks ??= {};
  const selectedEvents = new Set(profileName === "strict"
    ? ["SessionStart", "PreToolUse", "Stop"]
    : ["SessionStart", "Stop"]);
  // 用于从 V3.3 初版平滑移除已经安装、但不再需要的 PreCompact Hook。
  const managed = {
    ...(fragment.hooks ?? {}),
    PreCompact: [{
      matcher: "manual|auto",
  ...
  for (const [event, managedEntries] of Object.entries(managed)) {
    const desired = selectedEvents.has(event) ? (fragment.hooks[event] ?? []) : [];
    const desiredKeys = new Set(desired.map(canonical));
    const managedKeys = new Set(managedEntries.map(canonical));
    const existing = settings.hooks[event] ?? [];
    const kept = existing.filter((entry) => {
      const key = canonical(entry);
      if (managedKeys.has(key) && !desiredKeys.has(key)) {
        removed += 1;
        return false;
      }
      return true;
    ...
    for (const entry of desired) {
      if (!keptKeys.has(canonical(entry))) {
        kept.push(entry);
        keptKeys.add(canonical(entry));
        added += 1;
      }
    }
    if (kept.length) settings.hooks[event] = kept;
    else if (Object.hasOwn(settings.hooks, event)) delete settings.hooks[event];
  }
  ...
  if (existsSync(profilePath)) {
    ...
    let updated = profile.replace(/^(\s*hooks_installed:\s*).+$/m, "$1true");
    if (/^\s*hook_profile\s*:/m.test(updated)) {
      updated = updated.replace(/^(\s*hook_profile:\s*).+$/m, `$1"${profileName}"`);
    } else {
      const installedLine = updated.match(/^(\s*)hooks_installed:\s*true\s*$/m);
      if (installedLine) {
        updated = updated.replace(
          installedLine[0],
          `${installedLine[0]}\n${installedLine[1]}hook_profile: "${profileName}"`,
        );
      }
    }
    if (updated !== profile) {
  ...
  return {
    settings: relative(root, settingsPath).replaceAll("\\", "/"),
    profile: profileName,
    added,
    removed,
    installed: true,
...
function initProject(root, host) {
  if (host !== "codex") throw new Error("init-project 仅用于 Codex 插件版");
  const pluginRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..");
  const assetsDirectory = join(pluginRoot, "assets");
  const supportDirectory = workflowSupportDirectory(root, host);
  const profilePath = join(supportDirectory, "project-profile.yaml");
  const schemaPath = join(supportDirectory, "workflow-state.schema.json");
  const runsDirectory = join(supportDirectory, "workflow-runs");
  const profileSource = join(assetsDirectory, "project-profile.yaml");
  const schemaSource = join(assetsDirectory, "workflow-state.schema.json");
  if (!existsSync(profileSource) || !existsSync(schemaSource)) {
    throw new Error(`Codex 插件资源不完整: ${assetsDirectory}`);
  }
  mkdirSync(runsDirectory, { recursive: true });
  const profileCreated = !existsSync(profilePath);
  if (profileCreated) copyFileSync(profileSource, profilePath);
  copyFileSync(schemaSource, schemaPath);
  const ignorePath = join(runsDirectory, ".gitignore");
  if (!existsSync(ignorePath)) writeFileSync(ignorePath, "*\n!.gitignore\n", "utf8");
  return {
    host,
    support_directory: relative(root, supportDirectory).replaceAll("\\", "/"),
    profile: relative(root, profilePath).replaceAll("\\", "/"),
    profile_created: profileCreated,
    schema: relative(root, schemaPath).replaceAll("\\", "/"),
  };
}
...
function activeStates(root, host = "claude") {
  const paths = [
    ...walkForState(join(root, "openspec", "changes")),
    ...walkForState(join(workflowSupportDirectory(root, host), "workflow-runs")),
  ];
  return paths
    .map((path) => {
      try {
...
function hookSessionStart(root, host) {
  const active = activeStates(root, host);
...
function configuredHookProfile(root, host) {
  const profilePath = join(workflowSupportDirectory(root, host), "project-profile.yaml");
  if (!existsSync(profilePath)) return "default";
  const match = readFileSync(profilePath, "utf8").match(/^\s*hook_profile:\s*["']?([^"'\s#]+)["']?/m);
  return match?.[1] ?? "default";
}

function requestedPaths(input) {
  if (input.tool_input?.file_path) return [String(input.tool_input.file_path)];
  const command = String(input.tool_input?.command ?? "");
  return [...command.matchAll(/^\*\*\* (?:Add|Update|Delete) File:\s*(.+)$/gm)].map((match) => match[1].trim());
}

function hookPreToolUse(root, input, host) {
  if (host === "codex" && configuredHookProfile(root, host) !== "strict") return;
  const active = activeStates(root, host);
  ...
  const outsideAllowed = requested.filter((path) => {
...
function hookStop(root, input, host) {
  const active = activeStates(root, host);
  if (active.length !== 1) return;
  const { state } = active[0];
  const item = currentNode(state);
...
function hookCommand(event, root, host) {
  let input = {};
  try {
    const content = readFileSync(0, "utf8").trim();
  ...
  const projectRoot = findProjectRoot(input.cwd ?? root);
  if (event === "session-start") return hookSessionStart(projectRoot, host);
  if (event === "pre-tool-use") return hookPreToolUse(projectRoot, input, host);
  if (event === "stop") return hookStop(projectRoot, input, host);
...
function main() {
  ...
  const root = findProjectRoot(options.root ?? process.env.CODEX_PROJECT_ROOT ?? process.env.CLAUDE_PROJECT_DIR ?? process.cwd());
  ...
  if (command === "install-hooks") return print(installHooks(root, options));
  if (command === "init-project") return print(initProject(root, host));
  if (command === "hook") return hookCommand(subcommand, root, host);
  throw new Error("用法: workflow-runtime.mjs <route|compose|init-state|validate-state|next|transition|capture-spec-diff|review|reroute|archive-complete|install-hooks|init-project|hook>");
}
